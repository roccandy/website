...
| Image optimisation | Partially implemented | Many assets are handled well, and the homepage feature video has now been re-encoded from roughly `10 MB` down to about `3.1 MB`. Not every image/video asset has been fully audited yet. | Pending | Continue compressing oversized assets and checking performance, especially any remaining landing/product media. |
