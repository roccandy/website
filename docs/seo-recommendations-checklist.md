...
| Page speed / caching / minification / CDN | Partially implemented | Next.js + Vercel gives a strong baseline, public pages now use ISR-style caching instead of forced `no-store`, static assets have long-lived cache headers, and Next image delivery is configured for AVIF/WebP plus a longer cache TTL. Lab Lighthouse on `roccandy.vercel.app` is currently strong: home desktop `90`, home mobile `91`, wedding landing desktop `93`. | Pending | Re-run on the final production domain and use that result for launch sign-off; remaining measurable issue is a modest shared unused-JS chunk rather than a major image/server bottleneck. |
