...
- Preview hosts are still protected by default, but there is now a temporary preview crawl mode for staging SEO audits. Use it only long enough to run the crawl, then turn it back off so `vercel.app` URLs stay non-indexable.
