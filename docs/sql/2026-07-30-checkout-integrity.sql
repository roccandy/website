...
-- A provider transaction may cover several split order rows and several premade
-- product rows under one order number. The title distinguishes those product rows.
-- Historical imports are deliberately left intact while new checkout rows are protected.
