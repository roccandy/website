...
export default async function AdminInvoiceReviewPage({ params }: Params) {
  ...
  const invoiceStatus = groupedOrders.some((invoiceOrder) => invoiceOrder.square_invoice_status === "PAID")
    ...
    : groupedOrders.some((invoiceOrder) => invoiceOrder.square_invoice_status === "UNPAID")
      ? "Unpaid"
      : groupedOrders.some((invoiceOrder) => invoiceOrder.square_invoice_sent_at)
        ? "Sent"
        : "Not sent";
  const invoiceStatusClass =
    invoiceStatus === "Paid"
      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
      : invoiceStatus === "Unpaid" || invoiceStatus === "Sent"
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <h2 className="admin-page-title">Square invoice</h2>
          <div className="flex flex-wrap items-center gap-2 text-sm text-zinc-600">
            ...
            <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${invoiceStatusClass}`}>
              {invoiceStatus}
            </span>
          </div>
        </div>
