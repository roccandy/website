...
export async function createManagedPageAction(formData: FormData) {
  await requireAdminWriteAccess({ onDenied: "redirect", redirectTo: MANAGED_PAGES_ADMIN_PATH });

  const result = await saveManagedPage({
    slugPath: normalizeField(formData.get("slugPath")),
    ...
    ogImageUrl: normalizeField(formData.get("ogImageUrl")) || null,
